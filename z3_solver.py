...
class Z3Solver:
    ...
    def solve(self, checkpoint_handler):
        ...
        if last_best_waste is not None:
            self.solver.add(self.total_waste_var < last_best_waste)
    ...
    def check(self):
        self.last_check_result = self.solver.check()
    ...
    def _set_conservation_constraints(self):
        for m_src, l_src, k_src, node in _iterate_all_nodes(self.problem):
            ...
            self.solver.add(total_used <= total_produced)
    ...
    def _set_range_constraints(self):
        for m, l, k, node in _iterate_all_nodes(self.problem):
            ...
            for var in sharing_vars:
                ...
                self.solver.add(var >= 0, var <= effective_upper)
            ...
            for l, nodes in tree.items():
                if len(nodes) > 1:
                    for k in range(len(nodes) - 1):
                        ...
                        self.solver.add(total_input_k >= total_input_k1)
    ...
    def _set_objective_function(self):
        ...
        for m_src, l_src, k_src, node in _iterate_all_nodes(self.problem):
            ...
            self.solver.add(waste_var == total_prod - total_used)
